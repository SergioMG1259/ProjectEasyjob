...
export default {
  ...
  methods:{
    get_evidence_to_edit(id){
      this.serviceEvidence=new EvidencesApiServices(sessionStorage.getItem("token"))
      this.serviceEvidence.get_evidence_by_id(id).then(response=>{
    ...
    edit_evidence(){
      ...
      this.serviceEvidence.edit_evidence(this.id_evidence,data).then(this.close_dialg_edit)
    },
