<template>
  <!--Esta vista es para que un postulante aplique a un anuncio-->
  <v-container>
    <div class="scroll">
      <div v-for="item in list_notifications" :key="item.id">
        <v-card class="card-an" v-if="item.type=='declined'">
          ...
          <v-card-text>
            La empresa {{item.name_company}} no acepto su solicitud, porfavor revise el feedback para mejorar sus habilidades
          </v-card-text>
          <v-card-actions>
            <v-btn @click="Open_dialog_view_feedback">Ver feedback</v-btn>
          </v-card-actions>
        ...
        <v-card v-else-if="item.type=='accepted'" class="card-an">
          ...
          <v-card-text>
            La empresa {{item.name_company}} aceptó que trabajes con ellos
          </v-card-text>
        </v-card>
        <ViewFeedback :dialog="dialog_feedback" @close_feedback="Close_dialog_view_feedback" :feedback="item.message"></ViewFeedback>
      </div>
    </div>
  </v-container>
</template>
