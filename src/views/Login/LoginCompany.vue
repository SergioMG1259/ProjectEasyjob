  ...
  <v-container class="container-s">
    <v-row justify="center">
      <v-col cols="7">
        ...
        <v-row justify="center">
          <v-col class="d-flex justify-center">
            <v-btn to="/register-company" color="#9b59b6" elevation="4" large
            >Registrarse</v-btn
            >
          </v-col>
        </v-row>
        <v-row justify="center">
          <v-col class="d-flex justify-center">
            ...
            >Iniciar sesión como Postulante</v-btn
            >
          </v-col>
        </v-row>
      </v-col>
...
export default {
  ...
  methods:{
    validateLogin(){
      AccessCompanyServices.get_authorization({Email:this.email,Password:this.password}).then(response=>{
        ...
        if(response.data!=""){
          ...
          router.push(`/profile-company/${response.data.id}`)
        }
      })
